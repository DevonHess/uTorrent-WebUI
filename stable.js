...
var STable = new Class({
	...
	assignEvents: function() {
		...
		if (this.options.rowsSelectable) {
			...
			var a = {
				...
				"enter": function() {
					this.fireEvent("onDblClick", $(this.id + "-row-" + this.stSel).id.replace(this.idprefix, ""))
				}.bind(this),
				"down": function() {
					this.dBody.focus();
					row = $(this.id + "-row-" + this.stSel);
					table = this.dBody.querySelectorAll('[style=""]');
					if (!row) {
						this.selectRow({
					...
					} else if (row != table[table.length - 1]) {
						this.selectRow({
							"isRightClick": function() {
								return false;
							}
	...
	refreshRows: function() {
		...
		var b = this.activeId,
			l = this.rowData;
		this.tb.body.className = "sorted-" + this.sIndex;
	...
	setIcon: function(c, h) {
		...
		for (var f = 0, j = this.colIcon.length; f < j; ++f) {
			...
			if (this.colHeader[this.colIcon[f]].icon) {
				...
				if (utWebUI.settings["gui.color_progress_bars"] === undefined || utWebUI.settings["gui.color_progress_bars"]) {
					b.removeClass(e);
				}
