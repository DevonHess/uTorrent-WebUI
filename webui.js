...
var utWebUI = {
	...
	limits: {
		...
		minTableRows: 5,
		maxVirtTableRows: Math.ceil(window.innerHeight / 20) || 100,
	...
	themes: [
		...
		["Ubuntu", "./themes/ubuntu.css"],
		["Facebook", "./themes/facebook.css"],
		["Tumblr", "./themes/tumblr.css"],
		["reddit", "./themes/reddit.css"],
		["Yotsuba", "./themes/yotsuba.css"],
		["Steam", "./themes/steam.css"],
		["Discord", "./themes/discord.css"],
		["Rainbow", "./themes/rainbow.css"],
		["High Contrast", "./themes/highcontrast.css"]
	],
	defConfig: {
		showFeeds: true,
		notify: true,
		mobile: true,
		checkPort: false,
		root: 'ftp://ftp.devonhess.org/M:/Downloads/',
		space: 'http://devonhess.org/public/scripts/space.php?d=M',
		theme: "0",
		links: "Legacy%20WebUI%7Chttp%3A%2F%2Futorrent.devonhess.org%2Fgui%2Flegacy%2Findex.html%7C.%2Fimages%2Fut_small.png%0AUser%20Manual%7C.%2Fmanual%2Findex.html%7C.%2Fimages%2Fbook.png%0ADownloads%7Cftp%3A%2F%2Fftp.devonhess.org%2FM%3A%2FDownloads%2F%7C.%2Fimages%2Fuser%2Ffolder.png%0ADevon's%20Server%7Chttp%3A%2F%2Fdevonhess.org%2F%7C.%2Fimages%2Fuser%2Fdh.ico",
		search: "The%20Pirate%20Bay%7Chttps%3A%2F%2Fthepiratebay.org%2Fsearch%2F%0ABakaBT%7Chttps%3A%2F%2Fbakabt.me%2Fbrowse.php%3Fq%3D%0ANyaa%7Chttps%3A%2F%2Fnyaa.si%2F%3Fq%3D%0AAniDex%7Chttps%3A%2F%2Fanidex.info%2F%3Fq%3D%0AAniSearch%7Chttps%3A%2F%2Fanisearch.ru%2Ftorrents%3Fsearch%3D%0AExHentai%7Chttps%3A%2F%2Fexhentai.org%2Ftorrents.php%3Fsearch%3D%0AGoogle%7Chttp%3A%2F%2Fgoogle.com%2Fsearch%3Fq%3Dfiletype%253Atorrent%2B",
		showDetails: true,
	...
	resetConfigMap: {
		...
		grid: ["torrentTable", "trackerTable", "fileTable", "feedTable", "advOptTable", "ckMgrTable"],
		cookie: ["urlCookies"],
		extra: ["showFeeds", "notify", "mobile", "checkPort", "root", "space", "theme", "links", "search"]
	},
	torrentID: "",
	propID: "",
	rssfilterId: "",
	...
	init: function() {
		...
		this.config.lang = "";
		this.trtColOrderIdx = this.trtColDefs.map(function(a) {
			return (a.id == "order")
		}).indexOf(true);
		...
		this.traColDefs.each(function(b, a) {
			this.traColToggle(a, b.hidden, true)
		}, this);
	...
	remove: function(d) {
		...
		if ([this.settings.confirm_when_deleting, true].pick()) {
			...
			DialogManager.popup({
				title: (b > 1 ? "Remove Torrents" : "Remove Torrent"),
				icon: "dlgIcon-Delete",
	...
	loadList: function(b) {
						...
						n.each(function(p, o) {
							if (p != m.data[o]) {
								...
								if ("done" == this.trtColDefs[o].id) {
									...
									this.trtTable.updateCell(l, this.trtColOrderIdx, n)
									if (j && p == 1000) {
										this.notify(l);
									}
								}
		...
		(function(c) {
			...
			this.loadFeeds(Array.clone(c.rssfeeds), Array.clone(c.rssfeedm));
	...
	loadRSSFeedList: function() {
		...
		Object.each(this.rssfeeds, function(c, d) {
			var u = c[CONST.RSSFEED_URL].split("|")[1] || c[CONST.RSSFEED_URL].split("|")[0];
			b.grab(new Element("li", {
				...
				"class": "icon",
				"style": "background:" + (this.settings["gui.show_rss_favicons"] ? " url(" + favicon(u) + "); -webkit-mask-image: none;" : "")
			}), "top"))
		}, this);
		b.grab(a, "top");
	...
	notify: function(id) {
		if (this.config.notify) {
			if (Notification.permission === "granted") {
				new Notification('\u00B5Torrent WebUI', {
					icon: './images/ut.png',
					body: this.torrents[id][2] + " has finished downloading."
				}).onclick = function(event) {
					window.focus();
					this.close();
					utWebUI.trtTable.selectRow({
						"isRightClick": function() {
							return false;
						}
					}, {
						id: utWebUI.torrents[id][0]
					});
				}
			} else {
				Notification.requestPermission();
			}
			$("favicon").href = "./images/exclamation.png";
			var notification = setInterval(function() {
				if (document.hasFocus()) {
					$("favicon").href = "./images/ut_small.png";
					clearInterval(notification);
				} else {
					if ($("favicon").getAttribute("href") == "./images/ut_small.png") {
						$("favicon").href = "./images/exclamation.png";
					} else {
						$("favicon").href = "./images/ut_small.png";
					}
	...
	updateSpace: function(a) {
		if (this.config.space) {
			aj = new XMLHttpRequest();
			aj.onreadystatechange = function() {
				if (aj.readyState == 4 && aj.status == 200) {
					space = JSON.parse(aj.responseText);
					if (space.free != $("space").get("space")) {
						$("space").set('space', space.free);
						$("space").set("html", "").grab(progressBar(space.percent * 100));
						$("space").title = space.freeFormated.join(" ") + ' free of ' + space.totalFormated.join(" ");
					}
					$("space").style.display = "list-item";
				}
			}
			aj.open("GET", this.config.space, true);
			aj.send();
		}
	},
	update: function(a) {
	...
	loadLinks: function() {
		if (this.config.links) {
			var links = decodeURIComponent(this.config.links).split('\n');
			$("mainCatList-links").set("html","").grab(new Element("hr"));
			for (i = 0; i < links.length; i++) {
				if (!links[i]) {
					$("mainCatList-links").grab(new Element("hr"));
				} else {
					var name = links[i].split("|")[0] || "Link";
					var url = links[i].split("|")[1] || ".";
					var icon = links[i].split("|")[2] || favicon(url);
					$("mainCatList-links").grab(new Element("a", {
						"href": url
					}).grab(new Element("li", {
						"id": "link_" + url
					}).grab(new Element("span", {
	...
	loadFeeds: function(g, d) {
		var e = $("mainCatList-rss");
		$("cat_rss_c").set("text", Object.keys(this.rssfeeds).length);
		g.each(function(l) {
			var j = l[6].split("|")[0];
			var m = "feed_" + l[0];
			var u = l[6].split("|")[1] || j;
			if ((h = $(m))) {
				h.getElement(".alias").set("text", j);
				h.getElement(".icon").style.backgroundImage = (this.settings["gui.show_rss_favicons"] ? " url(" + favicon(u) + ");" : "")
			} else {
				e.grab(new Element("li#" + m).grab(new Element("span", {
					"class": "icon tag",
					"style": "background-image:" + (this.settings["gui.show_rss_favicons"] ? " url(" + favicon(u) + "); -webkit-mask-image: none; background-color: transparent;" : "")
				})).grab(new Element("span", {
					"class": "alias",
					text: j
				})));
				document.getElementById(m).addEvent("mousedown", function(h) {
	...
	loadLabels: function(g) {
		...
		g.each(function(l, i) {
			...
			if ((h = $(m))) {
				...
				e.grab(new Element("li#" + m).grab(new Element("span", {
					"class": "icon tag"
				})).appendText(j + " (").grab(new Element("span", {
					"class": "count",
					text: k
				})).appendText(")"))
			}
	...
	refreshSelectedRSSFeeds: function() {
		...
		for (var e in b) {
			...
			if (g > 0) {
				d.addClass("sel")
				$(e.split("rss")[1]).addClass("sel");
			} else {
				if (g < 0) {
					d.removeClass("sel")
					$(e.split("rss")[1]).removeClass("sel");
				}
			}
		}
	...
	addSettings: function(o, m) {
		var g = (function(q) {
			...
			this.traTable.setConfig({
				colSort: [i.trackerTable.sIndex, i.trackerTable.reverse],
				colMask: i.trackerTable.colMask,
				colOrder: i.trackerTable.colOrder,
				colWidth: i.trackerTable.colWidth
			});
		...
		if (isGuest) {
			...
			for (var h = 0, f = o.settings.length; h < f; h++) {
				...
				switch (n) {
					case "show_toolbar":
						this.config.showToolbar = c;
						break;
					case "show_details":
						this.config.showDetails = c;
						break;
					case "show_category":
						this.config.showCategories = c;
						break;
					case "show_status":
						this.config.showStatusBar = c;
						break;
					case "show_tabicons":
						this.config.showDetailsIcons = c;
						break;
					case "gui.find_pane":
						if (c) {
							$("mainCatList-find").style.display = "list-item";
						}
						break;
					case "gui.category_list_spaces":
						if (!c) {
							document.querySelectorAll("#mainCatList hr").forEach(function(h) {
			...
			this.settings.search_list = [this.settings.search_list,
				decodeURIComponent(this.config.search).replace(/\n/g, "\r\n")
			].pick();
			delete o.settings
		}
	...
	loadSettings: function() {
		...
		for (var b in this.settings) {
		...
		}["useSysFont", "showDetails", "showCategories", "showToolbar", "showStatusBar", "updateInterval", "lang", "notify", "showFeeds", "mobile", "checkPort", "root", "space", "theme", "links", "search"].each(function(e) {
			var f;
			if (!(f = $("webui." + e))) {
				return
			}
			...
			} else if (d && f.type == "textarea") {
				f.set("value", decodeURIComponent(d))
			} else {
				f.set("value", d)
			}
		}, this);
		...
		if (!this.config.showFeeds) {
			$("rssdownloader").show()
			$("mainCatList-rss").hide()
		}
		if (!this.config.mobile) {
			$("mobileStyle").media = "not all";
		}
		if (!this.config.space) {
			$("mainCatList-space").hide()
		}
		if (this.config.links) {
			this.loadLinks();
		} else {
			$("mainCatList-links").hide()
		}
	...
	setSettings: function() {
		...
		k = $("webui.notify").checked;
		if (this.config.notify != k) {
			this.config.notify = k;
			b = true
		}
		k = $("webui.showFeeds").checked;
		if (this.config.showFeeds != k) {
			this.toggleFeeds(k);
			b = true
		}
		k = $("webui.mobile").checked;
		if (this.config.mobile != k) {
			this.toggleMobile();
			b = true
		}
		k = $("webui.checkPort").checked;
		if (this.config.checkPort != k) {
			this.config.checkPort = k;
			b = true
		}
		k = $("webui.root").get("value");
		if (this.config.root != k) {
			this.config.root = k;
			b = true
		}
		k = $("webui.space").get("value");
		if (this.config.space != k) {
			this.config.space = k;
			this.toggleSpace();
			b = true
		}
		k = $("webui.theme").get("value");
		if (this.config.theme != k) {
			this.changeTheme(k);
			b = true
		}
		k = $("webui.links").get("value");
		if (this.config.links != k) {
			this.config.links = encodeURIComponent(k);
			this.loadLinks();
			this.toggleLinks();
			b = true
		}
		k = $("webui.search").get("value");
		if (this.config.search != k) {
			this.config.search = encodeURIComponent(k);
			b = true
		}
		var n = [];
		...
		for (var m in this.settings) {
			...
			switch (m) {
				...
				case "seed_time":
					...
					break;
				case "search_list":
					g = decodeURIComponent(this.config.search).split("\n").map(function(i) {
						return i.replace(/[\r\n]+/g, "")
					}).join("\r\n");
					break
			}
		...
		if (this.config.checkPort && f && (c !== f)) {
			this.endPeriodicUpdate();
	...
	associateMagnet: function() {
		var file = new Blob([
			'Windows Registry Editor Version 5.00\r\n\r\n' +
			'[HKEY_CLASSES_ROOT\\Magnet\\shell\\open\\command]\r\n' +
			'@="explorer \\"' + window.location.href + '#%1\\""'
		], {type: 'text'});
		var a = document.createElement('a');
		var url = URL.createObjectURL(file);
		a.href = url;
		a.download = '\u00B5Torrent WebUI.reg';
	...
	statusMenuShow: function(b) {
		var a = [
			...
			[CMENU_CHILD, L_("MM_OPTIONS"), [
				...
				[CMENU_SEP],
				["Associate with magnet URIs", this.associateMagnet.bind(this)]
			]],
			[CMENU_CHILD, L_("MM_HELP"), [
				["\u00B5Torrent Help", this.showManual.bind(this)],
				[L_("MM_HELP_UT_WEBPAGE"), openURL.pass(["http://www.utorrent.com/", null])],
				[L_("MM_HELP_UT_FORUMS"), openURL.pass(["https://forum.utorrent.com/", null])],
				[CMENU_SEP],
				[L_("MM_HELP_WEBUI_FEEDBACK"), openURL.pass(["https://forum.utorrent.com/topic/49588-%C2%B5torrent-webui/", null])],
				[CMENU_SEP],
	...
	trtSortCustom: function(b, d, c) {
		...
		switch (this.trtColDefs[b].id) {
			case "order":
				...
				var f = d[this.trtColStatusIdx][0],
					e = c[this.trtColStatusIdx][0];
	...
	showTrtMenu: function(d, g) {
		...
		if (this.config.root) {
			o = {
				open: ["Open", this.openTorrents.bind(this)],
				opencont: ["Open Containing Folder", this.openTorrentsCont.bind(this)],
			};
			b = b.concat([o.open, o.opencont, [CMENU_SEP]]);
		}
		b = b.concat([a.copymagnet, a.copy, [CMENU_SEP], a.forcestart, a.start, a.pause, a.stop, [CMENU_SEP], a.queueup, a.queuedown, a.label, [CMENU_SEP], a.remove, a.removeand, [CMENU_SEP], a.recheck, [CMENU_SEP], a.properties]);
		ContextMenu.clear();
		ContextMenu.add.apply(ContextMenu, b);
		ContextMenu.show(d.page)
	},
	openTorrents: function() {
		this.trtTable.getSortedSelection().each(function(b) {
			window.open(this.config.root + this.torrents[b][CONST.TORRENT_NAME]);
		}, this);
	},
	openFiles: function() {
		this.flsTable.getSortedSelection().each(function(c) {
			var a = c.match(/^(.*?)_.*$/)[1];
			if (this.flsTable.rowData[c]["data"][3] > 0) {
				if (this.flsTable.rows == 1) {
					window.open(this.config.root + this.flsTable.rowData[c]["data"][0]);
				} else {
					window.open(this.config.root + this.torrents[a][2] + "/" + this.flsTable.rowData[c]["data"][0]);
				}
			}
		}, this);
	},
	openTorrentsCont: function() {
		window.open(this.config.root);
	},
	openFilesCont: function() {
		this.flsTable.getSortedSelection().each(function(c) {
			var a = c.match(/^(.*?)_.*$/)[1];
			var f = this.flsTable.rowData[c]["data"][0].replace(/^(.+\\)?(.+)$/g, "$1");
			if (this.flsTable.rows == 1) {
				window.open(this.config.root);
			} else {
				window.open(this.config.root + this.torrents[a][2] + "/" + f);
			}
		}, this);
	},
	...
	addFile: function(g, d, p) {
		d = d || "";
		p = p || "";
		Array.each(Array.from(g.file), function(h) {
			switch (h.name.replace(/.*\.(.*)/, "$1")) {
				case "torrent":
					xhr = new XMLHttpRequest();
					formData = new FormData();
					xhr.open("POST", "./?token=" + this.TOKEN + "&" + "action=add-file&download_dir=" + d + "&path=" + p, true);
					formData.set("torrent_file", h);
					xhr.send(formData);
					break;
				case "btsearch":
					fr = new FileReader();
					fr.onload = function() {
						name = fr.result.replace(/^[^]*<shortname>(.*)<\/shortname>[^]*$/i, "$1");
						url = fr.result.replace(/^[^]*<url +?template="(.*)" *?\/>[^]*$/i, "$1").replace(/{searchterms}/ig, "%s");
						if (utWebUI.config.search.indexOf(encodeURIComponent(name + "|" + url)) == -1) {
							DialogManager.popup({
								title: "Add Search",
								icon: "dlgIcon-Search",
								message: "Would you like to add \"" + name + "\" to your search list?",
								buttons: [{
									text: L_("DLG_BTN_YES"),
									focus: true,
									click: function() {
										$("webui.search").value = name + "|" + url + "\n" + $("webui.search").value;
										utWebUI.setSettings();
									}
								}, {
									text: L_("DLG_BTN_NO")
	...
	loadTrackers: function() {
		this.traTable.keepScroll((function() {
			this.traTable.clearRows(true);
			var a = this.torrentID;
			if (a === this.trackerlist._ID_) {
				this.trackerlist.each(function(c, b) {
					this.traTable.addRow(this.traDataToRow(c), a + "_" + b)
				}, this)
			}
		}).bind(this));
		this.traTable.calcSize();
		this.traTable.resizePads();
		this.traTable.refresh()
	},
	getTrackers: function(c) {
		if (c === undefined) {
			c = this.torrentID
		}
		if (!c) {
		...
		var b = Date.now();
		this.request("action=getprops&hash=" + c, (function(e) {
			this.trackerlist.empty();
			var f = e.props[0].trackers.split(/\s{3,}/);
			this.trackerlist = f.map(function(c, i, a) {
				return [c,""];
			});
			this.trackerlist.unshift(
				["[DHT]", e.props[0].dht],
				["[Peer Exchange]", e.props[0].pex]
			);
			this.trackerlist._TIME_ = b;
			this.trackerlist._ID_ = c;
			this.loadTrackers()
		}).bind(this))
	},
	loadFiles: function() {
	...
	showFileMenu: function(k) {
		...
		var a = [];
		var o = [
			["Open", this.openFiles.bind(this, b)],
			["Open Containing Folder", this.openFilesCont.bind(this, b)],
			[CMENU_SEP]
		];
		if (this.config.root) {
			a = a.concat(o);
	...
	traColMove: function() {
		this.config.trackerTable.colOrder = this.traTable.colOrder;
		this.config.trackerTable.sIndex = this.traTable.sIndex;
		if (Browser.opera) {
			this.saveConfig(true)
		}
	},
	traColReset: function() {
		var a = {
			colMask: 0,
			colOrder: this.traColDefs.map(function(c, b) {
				return b
			}),
			colWidth: this.traColDefs.map(function(c, b) {
				return c.width
			})
		};
		this.traColDefs.each(function(c, b) {
			if (!!c.hidden) {
				a.colMask |= (1 << b)
			}
		});
		this.traTable.setConfig(a);
		Object.append(this.config.trackerTable, a);
		if (Browser.opera) {
			this.saveConfig(true)
		}
	},
	traColResize: function() {
		this.config.trackerTable.colWidth = this.traTable.getColumnWidths();
		if (Browser.opera) {
			this.saveConfig(true)
		}
	},
	traColToggle: function(d, c, b) {
		var a = 1 << d;
		if (c) {
			this.config.trackerTable.colMask |= a
		} else {
			this.config.trackerTable.colMask &= ~a
		}
		if (!b && Browser.opera) {
	...
	traDataToRow: function(a) {
		return this.traColDefs.map(function(b) {
			switch (a[1]) {
				case -1:
					a[1] = "not allowed";
					break;
	...
	traFormatRow: function(b, c) {
		var e = $chk(c);
		var a = (e ? (c + 1) : b.length);
		for (var d = (c || 0); d < a; d++) {
			switch (this.traColDefs[d].id) {
				case "name":
				case "status":
					break;
			}
	...
	toggleResolveIP: function() {
		...
		this.request("action=setsetting&s=resolve_peerips&v=" + (this.settings.resolve_peerips ? 1 : 0), (function() {
			if (this.torrentID != "") {
				this.getTrackers(this.torrentID, true)
			}
		}).bind(this))
	},
	traSelect: function(a, b) {
		if (a.isRightClick()) {
			this.showTrackerMenu.delay(0, this, a)
		}
	},
	showTrackerMenu: function(b) {
		if (isGuest || !b.isRightClick()) {
			return
		}
		var a = [
			[L_("ML_PROPERTIES"), this.showProperties.bind(this)],
			[CMENU_SEP],
			[L_("MENU_COPY"), this.traShowCopy.bind(this)]
		];
		ContextMenu.clear();
		ContextMenu.add.apply(ContextMenu, a);
		ContextMenu.show(b.page)
	},
	traShowCopy: function() {
		this.showCopy(L_("MENU_COPY"), this.traTable.copySelection())
	},
	traSort: function(b, a) {
		this.config.trackerTable.sIndex = b;
		this.config.trackerTable.reverse = a;
		if (Browser.opera) {
			this.saveConfig(true)
		}
	},
	...
	saveConfig: function(a, b) {
		...
		for (var key in this.config) {
			if (JSON.encode(this.config[key]) !== JSON.encode(this.defConfig[key])) {
				switch (key) {
					case "links":
					case "search":
						cookie[key] = encodeURIComponent(this.config[key]);
						break;
					default:
						cookie[key] = this.config[key];
						break;
				}
			}
		}
		this.request("action=setsetting&s=webui.cookie&v=" + JSON.encode(cookie), b || null, a || false)
	},
	...
	updateTitle: function() {
		...
		var remaining = 0;
		var size = 0;
		var selected = this.trtTable.getSortedSelection();
		...
		if (this.settings["gui.speed_in_title"]) {
			a = a.replace(/%s/, g_winTitle)
		} else {
			a = g_winTitle
		}
		if (selected.length > 0) {
			selected.each(function(b) {
				eta += (this.torrents[b][CONST.TORRENT_ETA] <= -1) ? 0 : this.torrents[b][CONST.TORRENT_ETA];
				remaining += this.torrents[b][CONST.TORRENT_REMAINING];
				size += this.torrents[b][CONST.TORRENT_SIZE];
			}, this);
			eta = (eta == 0) ? "" : (eta <= -1) ? " \u221E" : " " + eta.toTimeDelta();
			document.title = '[' + ((size - remaining) / size * 100).toFixedNR(1) + '%]' + eta + ' - ' + (selected.length == 1 ? this.torrents[selected[0]][CONST.TORRENT_NAME] : '|[' + selected.length + ' Torrents]|') + ' - ' + a;
		} else {
			document.title = a
		}
	},
	...
	toggleFeeds: function(a) {
		a = (a === undefined ? !this.config.showFeeds : !!a);
		$("mainCatList-rss")[a ? "show" : "hide"]();
		$("rssdownloader")[!a ? "show" : "hide"]();
		$("webui.showFeeds").checked = a;
		this.config.showFeeds = a;
		if (Browser.opera) {
			this.saveConfig(true)
		}
	},
	toggleLinks: function(a) {
		a = (a === undefined ? this.config.links : !!a);
		$("mainCatList-links")[a ? "show" : "hide"]();
		if (Browser.opera) {
			this.saveConfig(true)
		}
	},
	toggleMobile: function(a) {
		a = (a === undefined ? !this.config.mobile : !!a);
		$("mobileStyle").media = a ? "(max-device-width: 425px)" : "not all";
		$("webui.mobile").checked = a;
		if (a) {
			Notification.requestPermission();
		}
		this.config.mobile = a;
		if (Browser.opera) {
			this.saveConfig(true)
		}
	},
	toggleSpace: function(a) {
		a = (a === undefined ? !this.config.space : !!a);
		$("mainCatList-space")[!a ? "show" : "hide"]();
		if (Browser.opera) {
			this.saveConfig(true)
		}
	},
	changeTheme: function(a) {
		a = (a === undefined ? "0" : a);
		$("themeStyle").href = this.themes[a][1];
		this.config.theme = a;
		if (Browser.opera) {
			this.saveConfig(true)
		}
	...
	tableUseProgressBar: function(c) {
		...
		var d = this.traColDefs.filter(function(f) {
			return f.type == TYPE_NUM_PROGRESS
		}).map(function(f) {
			return f.id
		});
