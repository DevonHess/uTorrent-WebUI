...
function resizeUI(f, j) {
	...
	if (p) {
		...
		if (j) {
			...
			utWebUI.prsTable.resizeTo(n - 10, A - 2);
...
function setupUserInterface() {
	...
	$("mainInfoPane-peersTab").addEvent("mousedown", function(h) {
		if (h.isRightClick() && h.target.hasClass("stable-body")) {
			utWebUI.showPeerMenu(h)
		}
	});
	...
	_unhideSetting(["webui.lang", "enable_bw_management", "multi_day_transfer_mode", "total_uploaded_history", "total_downloaded_history", "total_updown_history", "history_period", "DLG_SETTINGS_7_TRANSFERCAP_12", "DLG_SETTINGS_A_ADVANCED_02", "webui.showToolbar", "webui.showCategories", "webui.showDetails", "webui.showStatusBar", "webui.maxRows", "webui.updateInterval", "webui.useSysFont", "dlgSettings-cookieDomain", "dlgSettings-cookieData", "dlgSettings-cookieSet"]);
...
function loadLangStrings(b) {
	...
	utWebUI.prsTable.setConfig({
		...
		colText: {
			ip: L_("PRS_COL_IP", 1),
			port: L_("PRS_COL_PORT", 1),
			client: L_("PRS_COL_CLIENT", 1),
			flags: L_("PRS_COL_FLAGS", 1),
			pcnt: L_("PRS_COL_PCNT", 1),
			relevance: L_("PRS_COL_RELEVANCE", 1),
			downspeed: L_("PRS_COL_DOWNSPEED", 1),
			upspeed: L_("PRS_COL_UPSPEED", 1),
			reqs: L_("PRS_COL_REQS", 1),
			waited: L_("PRS_COL_WAITED", 1),
			uploaded: L_("PRS_COL_UPLOADED", 1),
			downloaded: L_("PRS_COL_DOWNLOADED", 1),
			hasherr: L_("PRS_COL_HASHERR", 1),
			peerdl: L_("PRS_COL_PEERDL", 1),
			maxup: L_("PRS_COL_MAXUP", 1),
			maxdown: L_("PRS_COL_MAXDOWN", 1),
			queued: L_("PRS_COL_QUEUED", 1),
			inactive: L_("PRS_COL_INACTIVE", 1)
		}
	});
